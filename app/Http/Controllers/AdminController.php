...
class AdminController extends Controller
{
    ...
    public function admin_dashboard(Request $request)
    {
        ...
        $data['total_customers'] = User::where('user_type', 'customer')
        ...
        $data['total_inhouse_products'] = Product::where('approved', 1)
            ->where('published', 1)
            ->where('added_by', 'admin')
            ->count();

        $data['total_sellers_products'] = Product::where('approved', 1)
            ->where('published', 1)
            ->where('added_by', '!=', 'admin')
            ->count();
        ...
        $data['status_wise_sellers'] = Shop::select(
                'verification_status',
        ...
        $data['total_placed_order'] = Order::where('delivery_status', '!=', 'cancelled')
            ->count();

        $data['total_pending_order'] = Order::where('delivery_status', 'pending')
            ->count();

        $data['total_confirmed_order'] = Order::where('delivery_status', 'confirmed')
            ->count();

        $data['total_picked_up_order'] = Order::where('delivery_status', 'picked_up')
            ->count();

        $data['total_shipped_order'] = Order::where('delivery_status', 'on_the_way')
            ->count();
        ...
        $data['total_inhouse_sale'] = Order::where('seller_id', $admin_id)
            ->sum('grand_total');

        $data['payment_type_wise_inhouse_sale'] = Order::select(
                DB::raw("
                    CASE
                        WHEN payment_type = 'wallet' THEN 'wallet'
                        WHEN payment_type != 'cash_on_delivery' THEN 'others'
                        ELSE payment_type
                    END as payment_type
                "),
        ...
        $data['inhouse_product_rating'] = Product::where('added_by', 'admin')
            ->where('rating', '!=', 0)
            ->avg('rating');
    ...
    {
        $top_categories_products = DB::table(DB::raw("
            (
                SELECT
                    ...
                    od.sales,
                    od.total,
                    od.created_at order_detail_created,
                ...
                INNER JOIN (
                    ...
                    WHERE " .
                    (
                        $request->interval_type == 'all'
                            ? "1=1"
                            : "created_at >= NOW() - INTERVAL '1 {$request->interval_type}'"
                    ) .
                    "
    ...
    public function inhouse_top_categories(Request $request)
    {
        ...
        if ($request->interval_type != 'all') {
            $inhouse_top_category_query->where(
                'orders.created_at',
                '>=',
                DB::raw("NOW() - INTERVAL '1 {$request->interval_type}'")
            );
        }
        $inhouse_top_categories = $inhouse_top_category_query
    ...
    public function inhouse_top_brands(Request $request)
    {
        ...
        if ($request->interval_type != 'all') {
            $inhouse_top_brand_query->where(
                'orders.created_at',
                '>=',
                Carbon::now()->sub($request->interval_type, 1)
            );            
        }
        $inhouse_top_brands = $inhouse_top_brand_query
    ...
    public function top_sellers_products_section(Request $request)
    {
        ...
        $new_top_sellers_query = Order::select(
    ...
    {
        $top_brands_products = DB::table(DB::raw("
            (
                SELECT
                    products.id product_id,
                    products.name product_name,
                    ...
                    brands.logo,
                    ROW_NUMBER() OVER (
                        PARTITION BY products.brand_id
                        ORDER BY od.sales DESC
                    ) rn
    ...
    function clearCache(Request $request)
    {
        ...
        flash(translate('Cache all cleared successfully.'))->success();
        return back();
    }
}
