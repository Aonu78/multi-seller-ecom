    ...
    <div class="card">
        ...
        <div class="card-body">
            <div class="row gutters-5">
                ...
                @endphp
                @if ($order->seller_id == $admin_user_id || get_setting('product_manage_by_admin') == 1 || auth()->user()->user_type === 'admin')
            ...
            <div class="row gutters-5">
                <div class="col text-md-left text-center">
                    ...
                    function mask($string) {
                        $len = strlen($string);
                        if ($len <= 4) {
                            return str_repeat('*', $len); // mask fully if too short
                        }
                    ...
                    @if(json_decode($order->shipping_address))
                        ...
                        <address>
                            ...
                            {{ mask($addr->email) }}<br>
                            {{ mask($addr->phone) }}<br>
                            {{ mask($addr->address) }}, {{ mask($addr->city) }}, 
                            @if(isset($addr->state)) {{ mask($addr->state) }} - @endif 
                            {{ mask($addr->postal_code) }}<br>
                            {{ mask($addr->country) }}
                        </address>
                    @else
                        <address>
                            <strong class="text-main">
                                {{ mask($order->user->name) }}
                            </strong><br>
                            {{ mask($order->user->email) }}<br>
                            {{ mask($order->user->phone) }}<br>
                        </address>
                    @endif
                    ...
                    <tbody>
                        <tr>
                            ...
                            <td>
                                @php
                                    ...
                                    $commissionPercentage = get_setting('vendor_commission');
                                    $profit = ($subtotal * $commissionPercentage) / 100;
                                @endphp
                        ...
                        <tr>
                            ...
                            <td class="text-muted h5">
                                {{ single_price($order->grand_total + $profit) }}
                            </td>
                        </tr>
                    </tbody>
                </table>
