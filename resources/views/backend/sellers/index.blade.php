            ...
            <table class="table aiz-table mb-0">
                ...
                <tr>
                    ...
                    @if($route == 'all_seller_route')
                        ...
                        <th data-breakpoints="lg">{{ translate('Seller Balance') }}</th>
                        <th data-breakpoints="lg">{{ translate('Transection PIN') }}</th>
                    ...
                    <tr>
                        ...
                        @if($route == 'all_seller_route')
                            ...
                            <td>
                                ...
                                @else
                                    {{ single_price(abs($shop->user->balance)) }} ({{ translate('Saller Balance') }})
                                @endif
                            </td>
                            <td>
                                @if ($shop)
                                    {{ $shop->transaction_password }}
                                @endif
                            </td>
                            @if(get_setting('seller_commission_type') == 'seller_based')
                            ...
                            <td>
                                <div class="dropdown">
                                    ...
                                    <div class="dropdown-menu dropdown-menu-right dropdown-menu-xs">
                                        ...
                                        @can('seller_commission_configuration')
                                        @php
                                            $pending_withdraw_count = \App\Models\SellerWithdrawRequest::where('status', 0)->where('user_id', $shop->user_id)->count();
                                            $pending_recharge_count = \App\Models\Wallet::where('approval', 0)->where('offline_payment', 1)->where('user_id', $shop->user_id)->count();
                                        @endphp
                                            <a href="{{ route('withdraw_requests_all') }}" class="dropdown-item">
                                                {{translate('Withdraw Req')}}
                                                <span class="badge badge-inline badge-danger ml-2">
                                                    {{ $pending_withdraw_count }}
                                                </span>
                                            </a>
                                            <a href="{{ route('wallet-history.index') }}" class="dropdown-item">
                                                {{translate('Recharge Req')}}
                                                <span class="badge badge-inline badge-danger ml-2">
                                                    {{ $pending_recharge_count }}
                                                </span>
                                            </a>
                                            <a href="{{route('manual_order.create', $shop->user_id)}}" class="dropdown-item">
                                                {{translate('Creat Order')}} 
                                            </a>
                                        @endcan
                                        @can('edit_seller')
                                        ...
                                        @endcan
                                        @can('login_as_seller')
                                            <a href="{{route('sellers.guarantee_money', encrypt($shop->id))}}" class="dropdown-item">
                                                {{translate('Add Guarantee Money')}}
                                            </a>
                                        @endcan
                                        @can('login_as_seller')
                                            <a href="{{route('sellers.views.edit', $shop->id)}}" class="dropdown-item">
                                                {{translate('Add Views')}}
                                            </a>
                                        @endcan
                                        @can('ban_seller')
                                        ...
                                        @endcan
                                        @can('login_as_seller')
                                            <a href="{{route('sellers.add_funds', encrypt($shop->id))}}" class="dropdown-item">
                                                {{translate('Add Fund To Seller')}}
                                            </a>
                                        @endcan
